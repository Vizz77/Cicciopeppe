...
export const TestSubmissionModal = ({ opened, close, submitterId }: {
    ...
    // Assert the array type so TS knows it has .length and .map()
    const isResponseValid = Array.isArray(apiResult?.response?.results);
    const flagResponse = (isResponseValid ? apiResult?.response?.results : []) as any[];
                                ...
                                apiResult?.status == "ok" ? <Box>
                                    {
                                        apiResult.response?.ok ? <Box>
                                            ...
                                            <Alert ref={scrollExternalRefFirst} icon={<BsCardText />} title={<Title order={4}>Output from submitter {apiResult?.response?.warning ? "(some warning has been generated!)" : ""}</Title>} color={apiResult?.response?.warning ? "yellow" : "gray"} style={{ width: "100%", height: "100%", display: "flex" }}>
                                                <ScrollArea.Autosize mah={400}>
                                                    <Box style={{ whiteSpace: "pre" }} w={(scrollExternalRefFirst.current?.getBoundingClientRect().width - 70) + "px"}>
                                                        {apiResult.response?.output ? (apiResult.response.output as string) : <u>No output received from submitter</u>}
                                                    </Box>
                                                </ScrollArea.Autosize>
                                            </Alert>
                                            ...
                                            <Box>
                                                ...
                                                <Space h="lg" />
                                                <Alert ref={scrollExternalRefSecond} icon={<BsCardText />} title={<Title order={4}>{(apiResult.response?.error as string) ?? "No error message received"}</Title>} color="red" style={{ width: "100%", height: "100%", display: "flex" }}>
                                                    <ScrollArea.Autosize mah={400}>
                                                        <Box style={{ whiteSpace: "pre" }} w={(scrollExternalRefSecond.current?.getBoundingClientRect().width - 70) + "px"}>
                                                            {(apiResult.response?.output as string) ?? <u>No output received from submitter</u>}
                                                        </Box>
                                                    </ScrollArea.Autosize>
                                                </Alert>
                                            </Box>
